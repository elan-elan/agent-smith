...
Inputs: `location`, `cutoff_date` (`YYYY-MM-DD`), `output_path`; optional `crop_clip` for viewport `1600x1200`, default centered square `{ x: 410, y: 210, width: 780, height: 780 }`. Default framing should be a small neighborhood around the target, covering a few buildings; prefer about `500m` camera altitude for the first valid crop, then fall back wider when close historical tiles are blank or low-detail.
...
3. Set `targetDate = cutoff_date - 1 day`. Patch the current `/data=` payload: base64url-decode, replace an existing ISO date if present, otherwise insert bytes `2a100801120a{YYYY-MM-DD}1801` before marker `42020801420208004a` and increase the root varint length by the inserted field length. Try URL camera altitudes until the crop passes image checks:
   - Search-start crops: use `[500, 700, 1000, 1500, 2000, 2500, original]`, skipping values wider than the original camera except for ultra-close starts where `500m` should be tried first.
   - Direct-coordinate reused crops: use `[500, 1500, 2000, 2500, 700, 1000]` so common cases still try `500m` first, but hard historical tiles can jump to `1500m` before walking intermediate altitudes.
