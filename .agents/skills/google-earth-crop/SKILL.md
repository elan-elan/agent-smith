...
Inputs: `location`, `cutoff_date` (`YYYY-MM-DD`), `output_path`; optional `zoom_level`, and optional `crop_clip` for viewport `1600x1200`, default centered square `{ x: 410, y: 210, width: 780, height: 780 }`. Default framing should start at `zoom_level: 20`, which maps to about `150m` camera range and keeps the full output resolution instead of cropping a wider image. If zoom 20 fails because close historical tiles are blank or low-detail, try zoom 19 (`300m`), then zoom 18 (`600m`), then the intermediate recovery range (`1000m`), then the large recovery range (`1500m`). For tighter roof inspection, use `zoom_level: 21`, which maps to about `75m` camera range and falls back through zoom 20 and 19 before the recovery ranges.

For every crop, save both the PNG and a concise JSON sidecar next to it. Overlay the target as a red dot on the PNG by default. Append a small bottom strip from Google Earth's visible bottom status bar by default so the screenshot includes the actual imagery date label as pixels for later OCR; this is not parsed into JSON. The JSON sidecar should be a compact reproducibility manifest only: input location, output path, cutoff/target/selected timeline dates, final camera, zoom/range settings, viewport/clip, marker settings, and date-label strip settings. Do not include timing, image-quality analysis, retry history, marker pixel checks, screenshot byte counts, or other debug details in the saved sidecar. Do not label the injected historical timeline date as the actual imagery capture date; only include an imagery capture date in JSON if it is available from a reliable cross-platform browser or URL source.
...
- Zoom control: omit `--zoom-level` to use default zoom 20, or add `--zoom-level 21` for tighter roof-level crops. This patches both the Google Earth URL altitude (`a`) and camera range (`d`) fields; it is true source zoom, not a post-capture crop. When a zoom crop fails validation, the scripts try the next two lower zoom levels, then `--intermediate-fallback-camera-altitude` (`1000m` default), then `--large-fallback-camera-altitude` (`1500m` default).
...
- Visible date label: enabled by default by appending the Google Earth bottom status-bar strip to the saved PNG for later OCR. Add `--no-date-label` to skip.
...
3. Set `targetDate = cutoff_date - 1 day`. Patch the current `/data=` payload: base64url-decode, replace an existing ISO date if present, otherwise insert bytes `2a100801120a{YYYY-MM-DD}1801` before marker `42020801420208004a` and increase the root varint length by the inserted field length. Patch both URL camera altitude `a` and camera range `d`; changing only `a` records a new altitude but often does not visually zoom. For zoom-level crops, try the requested zoom, then the next two lower zoom levels, then the intermediate and large recovery ranges. Example for default zoom 20: `[150, 300, 600, 1000, 1500]`, corresponding to zoom 20, 19, 18, intermediate fallback, and large fallback. For `--preferred-camera-altitude` runs without `--zoom-level`, use the older adaptive altitude sequence.
