...
- Visible date label: enabled by default by capturing the Google Earth bottom status-bar strip, appending that strip below the saved crop, OCRing it, then overlaying the parsed image date at the top left when OCR succeeds. If OCR cannot parse a date, retry only the tiny bottom-strip screenshot and OCR, then still save the crop with the final captured strip appended for manual inspection. Add `--no-date-label` to skip strip capture/appending, OCR, and overlay; add `--no-date-ocr` to append the strip but skip OCR and therefore the overlay; or use `--date-ocr-retries N` / `--date-ocr-retry-wait-ms MS` to tune retries.
