...
- Visible date label: enabled by default by appending the Google Earth bottom status-bar strip to the saved PNG and OCRing the strip into the JSON sidecar. If OCR cannot parse a date, retry only the tiny bottom-strip screenshot and OCR before appending the final strip. Add `--no-date-label` to skip both the strip and OCR, `--no-date-ocr` to keep the strip but skip OCR, or `--date-ocr-retries N` / `--date-ocr-retry-wait-ms MS` to tune retries.
- OCR backfill: for existing crops with appended strips, run `node scripts/ocr_date_label.mjs --input path/to/crop.png --json path/to/crop.json` to add or refresh `parameters.dateLabel.ocr` without rerunning Google Earth.
